...
class FTupleSpec extends Specification with ScalaCheck {
  ...
  object ArbitraryTuples {
    ...
    implicit def arbTuple4[A: Arbitrary, B: Arbitrary, C: Arbitrary, D: Arbitrary]: Arbitrary[F.Tuple4[A, B, C, D]] =
      Arbitrary {
        for {
          a <- arbitrary[A]
          b <- arbitrary[B]
    ...
    implicit def arbTuple5[A: Arbitrary, B: Arbitrary, C: Arbitrary, D: Arbitrary, E: Arbitrary]
        : Arbitrary[F.Tuple5[A, B, C, D, E]] = Arbitrary {
      for {
        a <- arbitrary[A]
        b <- arbitrary[B]
        c <- arbitrary[C]
