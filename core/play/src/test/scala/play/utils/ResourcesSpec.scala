...
class ResourcesSpec extends Specification {
  ...
  "resources isDirectory" should {
    ...
    "return true for a directory resource URL with the 'bundle' and 'bundleresource' protocols" in {
      ...
      (isDirectory(osgiClassloader, urlBundle) must beTrue).and(isDirectory(osgiClassloader, urlBundleresource) must beTrue)
    }
    ...
    "return false for a file resource URL with the 'bundle' and 'bundleresource' protocols" in {
      ...
      (isDirectory(osgiClassloader, urlBundle) must beFalse).and(isDirectory(osgiClassloader, urlBundleresource) must beFalse)
    }
