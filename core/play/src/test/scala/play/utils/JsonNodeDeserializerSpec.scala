...
class JsonNodeDeserializerSpec extends BaseJacksonDeserializer("JsonNodeDeserializer") {
  def baseMapper(): ObjectMapper = {
    ...
    mapper.registerModule(JacksonJsonNodeModule)
    mapper
  }

  def baseMapper(feature: JsonParser.Feature): ObjectMapper =
    baseMapper().configure(feature, true)
}

class DefaultDeserializerSpec extends BaseJacksonDeserializer("default") {
  def baseMapper(): ObjectMapper = new ObjectMapper
  def baseMapper(feature: JsonParser.Feature): ObjectMapper = {
    baseMapper().configure(feature, true)
  }
}

abstract class BaseJacksonDeserializer(implementationName: String) extends Specification {
  ...
  s"A $implementationName Jackson ObjectMapper" >> {
    ...
    "read NaN as Double" >> {
      val json = """{ "value" : NaN }"""
      val mapper =
        baseMapper(JsonParser.Feature.ALLOW_NON_NUMERIC_NUMBERS)
          .configure(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS, true)
