    ...
    s"the cached action with hashResponse=${hashResponse}" should {
      ...
      "work with etag cache misses" in new WithCachedApplication() {
        override def running() = {
          ...
          status(action(FakeRequest("GET", "/c").withHeaders(IF_NONE_MATCH -> "*")).run()) must_== NOT_MODIFIED
      ...
      if (hashResponse) {
        "refresh only if content changed" in new WithCachedApplication() {
          override def running() = {
            var content = "1"
            val action  = cached(app)(_.uri)(Action(Results.Ok(content)))
            val resultA = action(FakeRequest("GET", "/a")).run()
            status(resultA) must_== 200
            val expires = header(EXPIRES, resultA).get
            ...
            import net.sf.ehcache._
            app.injector.instanceOf[CacheManager].clearAll()
            Thread.sleep(1000) // sleep to get a new EXPIRES
            // old etag doesn't expire
            val resultAA = useOldEtag
            status(resultAA) must_== NOT_MODIFIED
            // expires is updated
            header(EXPIRES, resultAA).get must_!= expires
