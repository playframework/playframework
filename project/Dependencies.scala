...
object Dependencies {
  ...
  val mockitoAll = "org.mockito"              % "mockito-core" % "3.6.28"
  ...
  val jpaDeps = Seq(
    ...
    "org.hibernate"                   % "hibernate-core"        % "5.4.25.Final" % "test"
  )
  ...
  val javaFormsDeps = Seq(
    "org.hibernate.validator" % "hibernate-validator" % "6.2.0.CR1",
  ...
  val testDependencies = Seq(junit, junitInterface, guava, findBugs, logback) ++ Seq(
    ...
    // local cache.
    ("org.seleniumhq.selenium" % "htmlunit-driver" % "2.45.0").excludeAll(
