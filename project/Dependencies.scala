...
object Dependencies {
  ...
  val akkaVersion: String     = sys.props.getOrElse("akka.version", "2.5.22")
  ...
  val playdocWebjarDependencies = Seq(
    "org.webjars" % "jquery"   % "3.4.0"        % "webjars",
  ...
  val testDependencies = Seq(junit, junitInterface, guava, findBugs, logback) ++ Seq(
    ...
    // local cache.
    ("org.seleniumhq.selenium" % "htmlunit-driver" % "2.35.0").excludeAll(
