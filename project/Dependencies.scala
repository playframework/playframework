...
object Dependencies {
  ...
  val mockitoAll = "org.mockito"              % "mockito-core" % "3.8.0"
  ...
  val testDependencies = Seq(junit, junitInterface, guava, findBugs, logback) ++ Seq(
    ...
    // local cache.
    ("org.seleniumhq.selenium" % "htmlunit-driver" % "2.48.0").excludeAll(
 ...
 *
 * Make sure Akka-HTTP has 2.12 as the FIRST version (or that scalaVersion := "2.13.5", otherwise it won't find the artifact
 *    crossScalaVersions := Seq("2.13.5", "2.11.12"),
 */
