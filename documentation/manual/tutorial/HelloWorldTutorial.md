...
Play APIs are available in both Java and Scala. The Framework uses [Pekko](https://akka.io) and [Pekko HTTP](https://doc.akka.io/docs/akka-http/10.2/index.html) under the hood. This endows Play applications with a stateless, non-blocking, event-driven architecture that provides horizontal and vertical scalability and uses resources more efficiently. Play projects contain Scala components, but because Play has a Java API, Java developers do not need to learn Scala to use Play successfully.
