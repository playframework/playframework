...
Of course, when deferring body parsing, the request body won't be parsed yet inside a `call(...)` method and therefore `request.hasBody()` will return `false` and `request.body()` will return `null`.
