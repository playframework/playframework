...
Play provides an alternative `HttpErrorHandler` implementation, named [`JsonHttpErrorHandler`](api/java/play/http/JsonHttpErrorHandler.html), which returns [Problem Details for HTTP APIs](https://www.rfc-editor.org/rfc/rfc9457.html) encoded as JSON. Responses use the `application/problem+json` media type and include the HTTP `status`, a `title`, and Play's `requestId`. Development-mode server errors also include diagnostic details and a stack trace.
