...
The interface through which Play handles these errors is [`HttpErrorHandler`](api/scala/index.html#play.api.http.HttpErrorHandler).  It defines two methods, `onClientError`, and `onServerError`.

## Supplying a custom error handler

A custom error handler can be supplied by creating a class in the root package called `ErrorHandler` that implements [`HttpErrorHandler`](api/scala/index.html#play.api.http.HttpErrorHandler), for example:
...
Checkout the full API documentation for [`DefaultHttpErrorHandler`](api/scala/index.html#play.api.http.DefaultHttpErrorHandler) to see what methods are available to override, and how you can take advantage of them.