...
* In order to guarantee that logged messages have had a chance to be processed by asynchronous appenders (including the TCP appender) and ensure background threads have been stopped, you'll need to cleanly shut down logback when your application exits. For details on a shutdown hook, see this [documentation](https://logback.qos.ch/manual/configuration.html#shutdownHook). Also you should add DelayingShutdownHook not default one. See this [post](https://jira.qos.ch/browse/LOGBACK-1090) for the reason.
