...
* `play.filters.cors.allowedOriginsAsRegex` - when enabled, treat entries in `allowedOrigins` as Java regular expressions instead of plain strings. Each expression must match the complete serialized origin; for example, the HOCON string `"https://[^\\.]+\\.example\\.net"` allows HTTPS origins with one subdomain label. The special `"*"` entry remains a wildcard. Since these expressions are evaluated against request headers, avoid patterns with excessive backtracking. The default is `false`.
