...
Play now uses the [Akka-HTTP](https://doc.akka.io/docs/akka-http/10.0/?language=scala) server engine as the default backend.  More detail about Play's integration with Akka-HTTP can be found [on the Akka HTTP Server page](https://www.playframework.com/documentation/2.6.x/AkkaHttpServer).  There is an additional page on [configuring Akka HTTP](https://www.playframework.com/documentation/2.6.x/SettingsAkkaHttp).
...
This automates most of the process of setting up HTTP/2. However, it does not work with the `run` command by default. See the [Akka HTTP Server page](https://www.playframework.com/documentation/2.6.x/AkkaHttpServer) for more details.
