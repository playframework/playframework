...
Please be aware that `Router.Attrs.HandlerDef` only exists with Routes generated by Play. 
For Routes generated using SIRD, the `request.attrs.get(Router.Attrs.HandlerDef)` call will return `None` in Scala and `null` in Java. Keep this in mind when creating filters.
