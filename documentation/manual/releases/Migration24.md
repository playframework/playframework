...
| [`GlobalSettings`](api/java/play/GlobalSettings.html) | [`HttpErrorHandler`](api/java/play/http/HttpErrorHandler.html), [`HttpRequestHandler`](api/java/play/http/HttpRequestHandler.html), and [`HttpFilters`](api/java/play/http/HttpFilters.html)| Read the details in the [[GlobalSettings|#GlobalSettings]] section below. |
...
* `GlobalSettings.onRouteRequest`: Unfortunately, this method cannot be easily refactored using the Play Java API. We are aware of the limitation, and we will be working on improving the status quo in the next releases.
...
* `GlobalSettings.onLoadConfig`: Pass the [`Configuration`](api/java/play/Configuration.html) instance to all classes needing to access the configuration. Or, even better, create a specialised configuration type that will expose only the settings needed by the class at hand. Use dependency injection to inject your specialised configuration class everywhere you need it.
...
* `GlobalSettings.configure` and `GlobalSettings.onLoadConfig`: Pass the [`Configuration`](api/scala/play/api/Configuration.html) instance to all classes needing to access the configuration. Or, even better, create a specialised configuration type that will expose only the settings needed by the class at hand. Use dependency injection to inject your specialised configuration class everywhere you need it.
