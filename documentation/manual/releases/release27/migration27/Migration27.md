...
That's why we added [`ApplicationEvolutions.finished`](api/scala/play/api/db/evolutions/ApplicationEvolutions.html#finished:Boolean) which indicates if the process of applying evolutions is finished or not. Only if that method returns `true` you can be sure that all evolutions scripts were executed successfully. `finished` will become `true` at some point eventually, because each time you apply or resolve evolutions scripts in `DEV` mode an application automatically restarts, re-initializing all it's modules.
