...
case class ObjectMapping5[T <: Product, A, B, C, D, E](apply: Function5[A, B, C, D, E, T], fa: (String, Mapping[A]), fb: (String, Mapping[B]), fc: (String, Mapping[C]), fd: (String, Mapping[D]), fe: (String, Mapping[E]), val key: String = "", val constraints: Seq[Constraint[T]] = Nil) extends Mapping[T] with ObjectMapping {

  val fieldA = fa._2.withPrefix(fa._1).withPrefix(key)
  val fieldB = fb._2.withPrefix(fb._1).withPrefix(key)
  val fieldC = fc._2.withPrefix(fc._1).withPrefix(key)
  val fieldD = fd._2.withPrefix(fd._1).withPrefix(key)
  val fieldE = fe._2.withPrefix(fe._1).withPrefix(key)

  def bind(data: Map[String, String]) = {

    merge(fieldA.bind(data), fieldB.bind(data), fieldC.bind(data), fieldD.bind(data), fieldE.bind(data)) match {
      case Left(errors) => Left(errors)
      case Right(values) => {
        applyConstraints(apply(
          values(0).asInstanceOf[A],
  ...
  def unbind(value: T) = {
    ...
    val c = fieldC.unbind(value.productElement(2).asInstanceOf[C])
    val d = fieldD.unbind(value.productElement(3).asInstanceOf[D])
    val e = fieldE.unbind(value.productElement(4).asInstanceOf[E])
    (a._1 ++ b._1 ++ c._1 ++ d._1 ++ e._1) -> (a._2 ++ b._2 ++ c._2 ++ d._2 ++ e._2)
  }
  ...
  val mappings = Seq(this) ++ fieldA.mappings ++ fieldB.mappings ++ fieldC.mappings ++ fieldD.mappings ++ fieldE.mappings
