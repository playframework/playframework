...
class NettyServer(appProvider: ApplicationProvider, port: Option[Int], sslPort: Option[Int] = None, address: String = "0.0.0.0", val mode: Mode.Mode = Mode.Prod) extends Server with ServerWithStop {
  ...
  private def loadKeyStore(): Option[KeyStore] =
  ...
  private def loadKeyManagers(keyStore: KeyStore) = {
    try {
      ...
      case NonFatal(e) => {
        Play.logger.error("Error loading HTTPS trust store", e)
        None
      }
    }
  }
  ...
  private def loadTrustManagers(keyStore: KeyStore): Option[Array[TrustManager]] = {
    ...
    System.getProperty("https.trustStore", "keystore") match {
      ...
      case "keystore" => {
        Play.logger.debug("Using configured key store as the trust store")
        try {
          ...
          case e: Exception => {
            Play.logger.error("Error loading trust managers", e)
            None
          }
        }
      }
        ...
        try {
          ...
          if (clazz.getInterfaces.toTraversable.exists(_ == classOf[X509TrustManager])) {
            try {
              ...
              Play.logger.info("Loaded TLS Trust Manager implementation " + clazz)
              res
            } catch {
              case e: InstantiationException => {
                Play.logger.error("could not instantiate " + className)
                None
              }
            }
          } else {
            Play.logger.error("TrustManager class " + className + " does not implement javax.net.ssl.TrustManager")
            None
          }
        } catch {
          case e: Exception => {
            Play.logger.error("Unknown trust store type, must be one of [keystore, noCA, default, class.Name]: "
