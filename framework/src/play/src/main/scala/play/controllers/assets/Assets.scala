...
import play.api._
import play.api.http.{ HttpErrorHandler, _ }
 ...
 */
private[assets] object StaticAssetsMetadata extends AssetsMetadata {

  @volatile private[assets] var instance: Option[AssetsMetadata] = None
  ...
  override private[assets] def digest(path: String) =
    delegate.digest(path)
  override private[assets] def assetInfoForRequest(request: RequestHeader, name: String) =
...
trait AssetsMetadata extends AssetsFinder {
  private[assets] def digest(path: String): Option[String]
  private[assets] def assetInfoForRequest(request: RequestHeader, name: String): Future[Option[(AssetInfo, Boolean)]]
}
...
class AssetsBuilder(errorHandler: HttpErrorHandler, meta: AssetsMetadata) extends BaseController {
   ...
   * @param file the file part extracted from the URL. May be URL encoded (note that %2F decodes to literal /).
   */
  private[assets] def resourceNameAt(path: String, file: String): Option[String] = {
