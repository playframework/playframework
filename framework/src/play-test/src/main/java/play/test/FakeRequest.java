...
public class FakeRequest {
    ...
    public FakeRequest withJsonBody(JsonNode node) {
        ...
        fake = new play.api.test.FakeRequest(Helpers.POST, fake.path(), new play.api.test.FakeHeaders(Scala.asScala(map).toSeq()), content, "127.0.0.1");
        return this;
    }
    ...
    public FakeRequest withJsonBody(JsonNode node, String method) {
        ...
        map.put("Content-Type", Scala.toSeq(new String[] {"application/json"}));
        AnyContentAsJson content = new AnyContentAsJson(play.api.libs.json.Json.parse(node.toString()));
        fake = new play.api.test.FakeRequest(method, fake.path(), new play.api.test.FakeHeaders(Scala.asScala(map).toSeq()), content, "127.0.0.1");
        return this;
    }
