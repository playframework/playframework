 ...
 */
abstract class Mock {
  private[test] lazy val cl = Thread.currentThread().getContextClassLoader()
  ...
  def createMock(mockPlugins: java.util.List[String], mockConfig: java.util.Map[String, String]): Application = {
    new Application(new File("."), cl, None, Mode.Dev) {
      ...
      val dynamicPlugins: List[String] = mockPlugins.asScala.toList
    }
  }
  /**
