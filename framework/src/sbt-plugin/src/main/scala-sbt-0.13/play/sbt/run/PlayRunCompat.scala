...
private[run] trait PlayRunCompat {
  ...
  def getSourcesFinder(watched: Watched, state: State): PlaySourceModificationWatch.PathFinder = () => watched.watchPaths(state).map(f => better.files.File(f.toPath)).toIterator
