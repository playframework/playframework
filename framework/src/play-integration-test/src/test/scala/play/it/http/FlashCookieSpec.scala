...
class FlashCookieSpec extends PlaySpecification
  ...
  val flashCookieApp: ComponentsBuilderApplicationFactory = {
    serveComponents.withRouter { components =>
   ...
   */
  final class CookieEndpoint(okEndpoint: OkHttpEndpoint) {
  ...
  class CookieEndpointOps(okEndpointOps: ServerEndpointOps[OkHttpEndpoint]) extends AdaptingServerEndpointOps[CookieEndpoint, OkHttpEndpoint](okEndpointOps) {
    override protected def withDelegate(newDelegate: ServerEndpointOps[OkHttpEndpoint]): CookieEndpointOps = new CookieEndpointOps(newDelegate)
    override protected def adaptBlock[A](block: CookieEndpoint => A): OkHttpEndpoint => A = { okEndpoint: OkHttpEndpoint =>
      block(new CookieEndpoint(okEndpoint))
    }
  }

  implicit class UseCookieRequest(applicationFactory: ApplicationFactory) {
    def useCookies: CookieEndpointOps = {
      val okEndpointOps: OkHttpEndpointOps = applicationFactory.useOkHttp
      new CookieEndpointOps(okEndpointOps)
    }
  }
  ...
  "the flash cookie" should {
    ...
    "be set for first request and removed on next request" in flashCookieApp
      .useCookies.forEndpoints { fcep: CookieEndpoint =>
    ...
    "allow the setting of additional cookies when cleaned up" in flashCookieApp
      .useCookies.forEndpoints { fcep: CookieEndpoint =>
        // Get a flash cookie
        val (response1, cookies1) = fcep.call("/flash", Nil)
        response1.code must equalTo(SEE_OTHER)
    ...
    "honor the configuration for play.http.flash.sameSite" in {
      ...
      "by not sending SameSite when configured to null" in flashCookieApp
        .withExtraConfig("play.http.flash.sameSite" -> null)
        .useCookies.forEndpoints { fcep: CookieEndpoint =>
      ...
      "by sending SameSite=Lax when configured with 'lax'" in flashCookieApp
        .withExtraConfig("play.http.flash.sameSite" -> "lax")
        .useCookies.forEndpoints { fcep: CookieEndpoint =>
          val (response, cookies) = fcep.call("/flash", Nil)
          response.code must equalTo(SEE_OTHER)
      ...
      "by sending SameSite=Strict when configured with 'strict'" in flashCookieApp
        .withExtraConfig("play.http.flash.sameSite" -> "lax")
        .useCookies.forEndpoints { fcep: CookieEndpoint =>
          val (response, cookies) = fcep.call("/flash", Nil)
          response.code must equalTo(SEE_OTHER)
    ...
    "honor configuration for flash.secure" in {
      ...
      "by making cookies secure when set to true" in flashCookieApp
        .withExtraConfig("play.http.flash.secure" -> true)
        .useCookies.forEndpoints { fcep: CookieEndpoint =>
          val (response, cookies) = fcep.call("/flash", Nil)
          response.code must equalTo(SEE_OTHER)
      ...
      "by not making cookies secure when set to false" in flashCookieApp
        .withExtraConfig("play.http.flash.secure" -> false)
        .useCookies.forEndpoints { fcep: CookieEndpoint =>
          val (response, cookies) = fcep.call("/flash", Nil)
          response.code must equalTo(SEE_OTHER)
          val cookie = cookies.find(_.name == flashCookieBaker.COOKIE_NAME)
