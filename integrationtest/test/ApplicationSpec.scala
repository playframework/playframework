  ...
  "execute index" in {
    ...
    result match {
      case SimpleResult(ResponseHeader(status, headers), body) => 
        ...
        val later:Promise[String] = (Iteratee.fold[String,String](""){ case (s,e) => s+e } <<: body).map(_.run)

        //clear mock from current scope
        globalMock.clearMock() 
        None must equalTo(None)
