...
trait CORSCommonSpec extends PlaySpecification {
  ...
  def commonTests = {
    ...
    "forbid an empty Access-Control-Request-Method header in a preflight request" in withApplication() { app =>
      val result = route(
        app,
        fakeRequest("OPTIONS", "/").withHeaders(ORIGIN -> "http://localhost", ACCESS_CONTROL_REQUEST_METHOD -> "")
      ).get
    ...
    val allowAllOrigins = Map("play.filters.cors.allowedOrigins" -> Seq("*"))

    "allow a cors request with any origin" in withApplication(conf = restrictOrigins) { app =>
