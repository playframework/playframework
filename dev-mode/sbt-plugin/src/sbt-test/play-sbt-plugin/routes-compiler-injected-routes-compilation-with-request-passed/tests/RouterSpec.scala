...
object RouterSpec extends PlaySpecification {
  ...
  "add route params to the request using a request attribute" in new WithApplication() {
    override def running() = {
      val result = route(
        implicitApp,
        FakeRequest(GET, "/23/abc?page=2&someOtherParam=34&page=4&books=Moby%20Dick&books=War%20and%20Peace")
      ).get
