...
public class Helpers implements play.mvc.Http.Status, play.mvc.Http.HeaderNames {
  ...
  /**
   * Creates a new JavaContextComponents using Configuration.reference and Enviroment.simple as
   * defaults
   *
   ...
   * @param requestBuilder the request builder
   * @param timeout the timeout
   * @deprecated as of 2.6.0. Use {@link Helpers#routeAndCall(Application, Class, RequestBuilder,
   *     long)}.
   * @see GuiceApplicationBuilder
   * @return the result
   */
  ...
  public static Result routeAndCall(
    ...
    try {
      ...
      return routes
          .route(request)
          .map(handler -> invokeHandler(app.getWrappedApplication(), handler, request, timeout))
          .orElse(null);
    } catch (RuntimeException e) {
      throw e;
    } catch (Throwable t) {
  ...
  public static Result route(Application app, RequestBuilder requestBuilder, long timeout) {
    final scala.Option<scala.concurrent.Future<play.api.mvc.Result>> opt =
        play.api.test.Helpers.jRoute(
            app.getWrappedApplication(), requestBuilder.build().asScala(), requestBuilder.body());
